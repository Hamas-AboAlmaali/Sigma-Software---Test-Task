...
const addCandidate = async (firstName, lastName, phoneNumber, email, timeInterval, linkedInURL, GitHubURL, comment) => {
  ...
  if (rows.length > 0) {
    return await db.execute(
      ...
      [firstName, lastName, phoneNumber, linkedInURL, GitHubURL, comment, timeInterval[0].$H, timeInterval[1].$H, email]
    )
  } else {
    return await db.execute(
      ...
      [firstName, lastName, phoneNumber, email,timeInterval[0].$H, timeInterval[1].$H, linkedInURL, GitHubURL, comment]
    )
  }
